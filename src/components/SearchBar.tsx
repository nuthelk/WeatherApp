...
const SearchBar = () => {
  ...
  return (
    <form onSubmit={(e) => handleClick(e)} className='w-full flex  gap-4 border-2 border-slate-200 rounded-lg justify-between sm:w-[500px] bg-white'>
        <input type="text" className='p-2 w-[65%] rounded-lg outline-none text-black capitalize' value={input} onChange={handleChange} placeholder='ciudad' required/>
        <button className='bg-slate-400 p-2 w-24 rounded-lg dark:bg-gray-900 font-medium' type='submit'>Buscar</button>
    </form>
  )
}
